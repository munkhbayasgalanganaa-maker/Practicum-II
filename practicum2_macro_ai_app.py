...
st.set_page_config(page_title="Practicum II Macro Intelligence", layout="wide")
...
TABLE_SPECS = {
    ...
    "step7_links": {
        ...
        "required": ["category", "best_lag_months", "best_corr", "abs_corr"],
    },
    "step7_event": {
...
def render_header() -> None:
    st.title("Practicum II: Macro-Clean Intelligence Dashboard")
...
def render_sidebar() -> tuple[Path, dict[str, LoadedTable]]:
    ...
    processed_dir_input = st.sidebar.text_input("Processed Data folder", value=str(DEFAULT_PROCESSED_DIR))
    ...
    st.sidebar.caption("If auto-load fails, upload CSVs generated from the notebook.")

    for name, spec in TABLE_SPECS.items():
        upload = st.sidebar.file_uploader(f"Upload {spec['filename']}", type=["csv"], key=f"upload_{name}")
        if upload is not None:
            try:
                df = read_csv_from_bytes(upload.getvalue())
                loaded[name] = LoadedTable(name=name, df=df, source=f"uploaded: {upload.name}")
            except Exception as exc:
                st.sidebar.error(f"{spec['filename']} upload failed: {exc}")
...
def render_step7(loaded: dict[str, LoadedTable]) -> None:
    ...
    if links_obj.df is not None and {"category", "abs_corr"}.issubset(links_obj.df.columns):
        ...
        fig_links = px.bar(
            ...
            text="abs_corr",
            title="GPR -> Category Lead-Lag Strength",
        )
